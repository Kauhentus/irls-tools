...
const main = async () => {
    ...
    for (let i = 0; i < features.length; i++) {
        ...
        let row = []; // each row = different color, each column = different position
        for (let j = 0; j < features.length; j++) {
            // distance from i color to j position on the grid
            let position = [
                j / grid[0] | 0,
                j % grid[0]
            ];
            positions.push(position);
            let distance = Math.sqrt((feature[0] - position[0]) ** 2 +
    ...
    //             255
    //         })`;
    //         ctx.fillRect(x_corner, y_corner, square_size[0], square_size[1]);
    //     }
    // }
